...
public class XLII_CampaignPlugin extends BaseCampaignPlugin {
    ...
    public PluginPick<InteractionDialogPlugin> pickInteractionDialogPlugin(
        ...
        // --- Daikyu escalation: swap Draconis torpedo loadout if player fields nuclear weapons ---
        if (interactionTarget instanceof CampaignFleetAPI) {
            CampaignFleetAPI fleet = (CampaignFleetAPI) interactionTarget;
            if (DraconisWeaponEscalationMonitor.isEnabled()
                    && fleet.isHostileTo(Global.getSector().getPlayerFleet())
                    && (Factions.DRACONIS.equals(fleet.getFaction().getId())
                        || Factions.FORTYSECOND.equals(fleet.getFaction().getId()))
                    && DraconisWeaponEscalationMonitor.playerHasNuclearWeapons()) {
                DraconisWeaponEscalationMonitor.applyEscalationTo(fleet);
            }
        }
