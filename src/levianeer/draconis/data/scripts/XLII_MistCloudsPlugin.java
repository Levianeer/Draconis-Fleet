...
public class XLII_MistCloudsPlugin implements EveryFrameCombatPlugin {
    ...
    private static final Color NEBULA_COLOR_BASE = new Color(125, 130, 142, 130); // dark blue-grey mid layer
    private static final Color NEBULA_COLOR_WISP = new Color(165, 170, 180, 90);  // lighter blue-grey edges
    ...
    private InitStats computeInitStats() {
        ...
        // Pass 1: locate the first XLII ship to determine team side
    ...
    private void performPeriodicScan() {
        ...
        // Update side detection and spawn angle
    ...
    private void checkForNewClouds() {
        ...
        for (Map.Entry<String, Object> entry : customData.entrySet()) {
            ...
            if (key.startsWith("XLII_MIST_SPAWN_")) {
    ...
    private void spawnCloudMissile(Vector2f toward) {
        ...
        Vector2f spawnPos = new Vector2f(
            toward.x + (float)FastTrig.cos(Math.toRadians(angle)) * spawnDistance,
            toward.y + (float)FastTrig.sin(Math.toRadians(angle)) * spawnDistance
        );
