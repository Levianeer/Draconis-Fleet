...
import levianeer.draconis.data.campaign.intel.events.DraconisHostileActivityManager;
import levianeer.draconis.data.campaign.intel.events.NexerelinRaidListener;
...
public class XLII_ModPlugin extends BaseModPlugin {
    ...
    public void onGameLoad(boolean newGame) {
        ...
        // If Nexerelin is present, add the raid listener and AI core targeting system
        if (hasNexerelin) {
            Global.getLogger(this.getClass()).info("Registering Nexerelin integration systems:");

            // Core theft system (listens for completed raids)
            Global.getSector().addScript(new NexerelinRaidListener());
            Global.getLogger(this.getClass()).info("  - Raid listener for AI core theft");

            // AI core targeting system (monitors markets and biases raid selection)
            ...
            Global.getLogger(this.getClass()).info("Nexerelin integration complete");
        } else {
